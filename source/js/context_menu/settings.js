...
function openSettings(parent) {
  ...
  children[4].innerHTML = `
    <option lang="ar-SA">العربية</option>
    <option lang="en-US">English</option>
  `;
  ...
  children[11].style.float = "var(--float-start)";
  ...
  children[12].style.float = "var(--float-end)";
  ...
  children[17].addEventListener("click", (event) => {
    document.getElementById("main-window").classList.toggle("gpu-capable");
  ...
  children[19].addEventListener("click", (event) => {
    document.getElementById("main-window").classList.toggle("gamermode");
  ...
  children[21].type = "color";
  children[21].addEventListener("change", (event) => {
    document.getElementById("main-window").style.setProperty("--accent-color", children[22].value);
    localStorage.setItem("accentColor", children[22].value);
  });
  children[22].setAttribute("data-l10n-id", "settings-accentColor");
